...
<nav className={`hidden ${scrolling ? "px-8 py-2" : "shadow-lg border px-8 py-2 rounded-full"} lg:flex items-center space-x-8 text-gray-800 font-medium`}>
  ...
  <div className="flex items-center space-x-4  rounded-full px-2  bg-secondary">
    <a href="https://wa.me/+919519202509" target="_blank" rel="noopener noreferrer" className="text-white pl-2 text-2xl hover:scale-110 transition">
      <FaWhatsapp />  
    </a>
    <a href="tel:+919519202509" className="text-white  text-xl hover:scale-110 transition">
      <FaPhoneAlt />
    </a>
    <Link href="/contact-us" className="bg-[#e00909] text-white px-4 py-2 m-1 rounded-full hover:bg-secondary  hover:text-white transition">
      Get in touch
    </Link>
  </div>
</nav>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-white z-50 p-6">
          <div className="flex items-center justify-between mb-6">
            <Link href="/" className="text-2xl font-bold text-[#e00909]">Difmo</Link>
          ...
          <nav className="flex flex-col gap-4 text-lg font-medium">
            ...
            <Link href="/contact-us" onClick={() => setIsMenuOpen(false)} className="bg-[#e00909] text-white px-4 py-2 rounded-lg text-center">
              Get in touch
            </Link>
          </nav>
        </div>
...
const HeaderLink = ({ href, isActive, children }) => (
  ...
  >
    {children}
    {isActive && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#e00909]" />}
  </Link>
);
